...
const DonationBanner = () => (
  <section className="mt-20 py-16 px-6 bg-[#000666] text-white text-center rounded-[2.5rem] shadow-2xl relative overflow-hidden">
    <div className="relative z-10">
      ...
      <a href="https://www.buymeacoffee.com/" target="_blank" className="inline-flex items-center gap-3 bg-[#FFDD00] text-black px-10 py-5 rounded-2xl font-black text-xl hover:scale-105 transition-all shadow-xl">
        ☕ Buy Me a Coffee
      </a>
    </div>
  </section>
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] text-[#1A202C] font-sans flex">
      <aside className="hidden lg:flex flex-col fixed h-screen w-64 bg-[#F1F4F6] border-r border-slate-200 p-8 z-50">
        ...
        <nav className="flex-1 space-y-4 text-slate-900">
          ...
          <button onClick={() => {setActiveTab('database'); setSelectedCase(null)}} className={`flex items-center gap-3 text-xs font-black uppercase tracking-widest ${activeTab==='database'?'text-black':'text-slate-400'}`}><Database className="w-4 h-4"/> アーカイブ</button>
        </nav>
        ...
        {selectedCase ? (
          <div className="max-w-3xl mx-auto animate-in slide-in-from-bottom-4 space-y-8">
            ...
            <div className="bg-white p-12 border border-slate-100 shadow-2xl space-y-8">
              <h2 className="font-serif text-5xl font-black">{selectedCase.what || "タイトルなし"}</h2>
              <div className="text-red-700 font-bold border-y py-4 text-sm">処分結果：{selectedCase.punishment || "調査中"}</div>
              <p className="font-serif text-xl leading-relaxed text-slate-700">{selectedCase.summary || "詳細な記録はありません。"}</p>
            </div>
          </div>
        ) : (
            ...
            {activeTab === 'paper' ? (
              <div className="space-y-12">
                <div className="border-y-4 border-black py-6 flex justify-between items-end">
                  ...
                  <div className="hidden md:block text-right font-serif opacity-40"><p className="text-xl font-bold text-slate-900">The Digital Broadsheet</p></div>
                </div>
                ...
                {loading ? <p className="font-serif italic text-slate-400">Loading Database...</p> : (
                  cases.length > 0 ? (
                    ...
                    <div className="py-20 text-center border-2 border-dashed border-slate-200 rounded-3xl">
                      <p className="font-serif text-xl text-slate-400 italic">現在、最新の不祥事案をデータベースから読み込んでいます。<br/>反映までしばらくお待ちください。</p>
                    </div>
                  )
                )}
                <DonationBanner />
                ...
                <div className="grid gap-4">
                  {cases.map(c => (
                    <div key={c.id} onClick={() => setSelectedCase(c)} className="bg-white p-6 border border-slate-100 shadow-sm hover:shadow-md cursor-pointer flex justify-between items-center transition-all">
                      <div><div className="text-[10px] font-bold text-slate-400 uppercase mb-1">{c.date} | {c.location}</div><div className="font-bold text-slate-800">{c.what}</div></div>
